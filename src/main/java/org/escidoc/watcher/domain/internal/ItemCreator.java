...
public class ItemCreator implements Consumer {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(ItemCreator.class);
  ...
  private void tryCreateItem(final FileEvent event) {
    try {
      login();
      final String type = URLConnection.guessContentTypeFromName(event.getFullPath().getFileName().getFileName().toString());
      LOG.debug("mime/type guessed: " + type);

      final Item newItem = createItem(putFileInStagingServer(loadFile(event)),
          event.getFullPath().getFileName());

      LOG.debug("a new Item is created with an ID: " + newItem.getObjid());
  ...
  private URL putFileInStagingServer(final InputStream stream)
    ...
    LOG.debug("putting the file to staging server: " + stream);
  ...
  private InputStream loadFile(final FileEvent event) throws IOException {
    return Files.newInputStream(event.getFullPath(), StandardOpenOption.READ);

  }

  private Item createItem(final URL contentUrl, Path path)
      throws EscidocException, InternalClientException, TransportException {
    final Item created = itemClient.create(new ItemBuilder.Builder(
        new ContextRef(config.getContextId()), new ContentModelRef(
            config.getContentModelId())).withContent(contentUrl).withName(path).build());
    return created;
  }

  private void login() throws EscidocClientException {
    final Authentication authentication = new Authentication(serviceAddress,
        config.getLoginName(), config.getPassword());
